...
User Function RelVenBE
	...
	oPrn := TMSPrinter():New("Relatorio Pedido de Venda - Por Emissão Pedido")
...
/*/
+----------+------------+----------------------------+------+------------+
|Funcao    | ImpDados   | Márcio - Aptado por Myrella| Data | 01/08/14   |
+----------+------------+----------------------------+------+------------+
...
Static Function ImpDados
	...
	cQuery:= "SELECT C6_FILIAL, C6_NUM, C6_SUGENTR, C6_DATFAT, C6_NOTA, C6_SERIE, A1_NOME, C6_LOTECTL, C6_DESCRI, C6_CLASS, C6_COMPLIQ, "
	...
	cQuery+= " INNER JOIN SM2050 ON M2_DATA = C6_SUGENTR"

	// BUSCAR NOME DO CLIENTE PARA FACILITAR EXIBICAO.
	cQuery+= " INNER JOIN SA1050 ON A1_COD = C6_CLI AND A1_LOJA = C6_LOJA"

	cQuery+= " WHERE C6_FILIAL BETWEEN '"+mv_par01+"' AND '"+mv_par02+"' AND C6_SUGENTR BETWEEN '"+DTOS(mv_par03)+"' AND '"+DToS(mv_par04)+"'"
	cQuery+= " AND SC6050.D_E_L_E_T_ = '' AND SM2050.D_E_L_E_T_ = '' AND C6_PRODUTO BETWEEN '"+mv_par05+"' AND '"+mv_par06+"' AND C6_CLI BETWEEN '"+mv_par07+"' AND '"+mv_par08+"'"
	...
	cQuery+= " ORDER BY C6_FILIAL,C6_SUGENTR, C6_LOTECTL"
	...
	While (!Eof())
		...
		oPrn:Say( nLin , 0310 , Substr(VDAS->C6_SUGENTR,7,2)+"/"+Substr(VDAS->C6_SUGENTR,5,2)+"/"+Substr(VDAS->C6_SUGENTR,1,4),oFont1)	
		// Ocultar por não é para sair a nota fiscal
...
Static Function ImpCabec()
	...
	oPrn:Say( 0030 , 0850 , "Relatorio Pedido de Venda - Por Emissão Pedido" , oFont3 )
...
Static Function ImpCabec1()
	...
	oPrn:Say( nLin , 0310 , "Dat. Emi."											,oFont2)
