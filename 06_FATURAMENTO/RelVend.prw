...
User Function RELVEND
	...
	oPrn := TMSPrinter():New("Relatorio de Vendas por Bloco")
...
/*/
+----------+------------+----------------------------+------+------------+
|Funcao    | ImpDados   | Marcio Filho               | Data | 27/05/08   |
+----------+------------+----------------------------+------+------------+
...
Static Function ImpDados
	...
	cQuery:= "SELECT DISTINCT C6_FILIAL, C6_NUM, C6_ITEM,C6_DATFAT, C6_NOTA, C6_SERIE, A1_NOME, C6_LOTECTL, C6_DESCRI, C6_CLASS, C6_COMPLIQ, "
	cQuery+= "			C6_ALTLIQ, C6_LARGLIQ, C6_QTDVEN, C6_UM, C6_PRCVEN*M2_MOEDA2 AS PRCVEN, C6_PRCVEN*1 AS PRCVEN1, "
	cQuery+= "			C6_VALOR*M2_MOEDA2 AS TOTAL, C6_VALOR*1 AS TOTAL1 ,C6_CF "
	cQuery+= "FROM SC6050 "
	...
	//Não sair notas de entrega futura - Solicitação Airton - 01-06-2016
	cQuery+= "AND F4_CF <> '5922'"
	cQuery+= "AND F4_CF <> '5122'"
	...
	Else 
		...
		cQuery+= " AND C6_CLASS IN ('','1ª','CO','2ª','ST')"
	...
	//MYRELLA - AJUSTE REALIZADO PARA ATENDER A REMESSA PARA INDUSTRIALIZACAO POR CONTA E ORDEM
	cQuery+= " UNION ALL"	
	cQuery+= " SELECT C6_FILIAL, C6_NUM, C6_ITEM,C6_DATFAT, C6_NOTA, C6_SERIE, A1_NOME, C6_LOTECTL, C6_DESCRI, C6_CLASS, C6_COMPLIQ, "
	cQuery+= "			C6_ALTLIQ, C6_LARGLIQ, C6_QTDVEN, C6_UM, C6_PRCVEN*M2_MOEDA2 AS PRCVEN, C6_PRCVEN*1 AS PRCVEN1, "
	cQuery+= "			C6_VALOR*M2_MOEDA2 AS TOTAL, C6_VALOR*1 AS TOTAL1 ,C6_CF "
	cQuery+= "FROM SC6050 "
	...
	While (!Eof())
		...
		//Aparececer NFs de remesa com o valor 0	
		//	cCF 	:=  AllTrim(VDAS->C6_CF)							

		//If (C6_CF == '5922')// .OR. (VDAS->C6_CF == 6922))
		//	oPrn:Say( nLin , 2470 , "0"+" x "+"0"+" x "+"0"+" = "+"0"		,oFont1)
		//Else
		//	oPrn:Say( nLin , 2470 , cComp+" x "+cAlt+" x "+cLarg+" = "+cQuant		,oFont1)	
		//EndIf	
...
Static Function ImpCabec()
	...
	oPrn:Say( 0030 , 0850 , "Relatorio de Vendas Por Bloco" , oFont3 )
