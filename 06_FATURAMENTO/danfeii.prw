...
#DEFINE MAXITEMC   035                                                // Máxima de caracteres por linha de produtos/serviços //SIGAWISE BRUNO LAGE
...
/*
****************************************************************
DATA DO ARQUIVO: Quarta-feira, 18/10/2019, 
DATA DA IMPLEMENTAÇÃO: 27/11/2019
****************************************************************
*/
/*/
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	For nZ := 1 To nLenDet
		...
		//Bruno exportação
		//***************************************************************
		If ( oDet[nX]:_Prod:_uCom:TEXT <> oDet[nX]:_Prod:_uTrib:TEXT )
		...
		nMaxCod := 13 //MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 50) bruno sigawise
		...
		SubStr(aAux[1], 1, 11),; //SubStr(aAux[1], 1, PosQuebrVal(aAux[1])),;
		...
		While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6]) .Or. !Empty(cUnCom) // bruno 
			...
			*
			*/
			if Empty(cAux) .and. Empty(cAuxItem) .and. Empty(aAux[1]) .and. Empty(aAux[2]) .and. Empty(aAux[3]) .and. Empty(aAux[4]) .and. Empty(aAux[5]) .and. Empty(aAux[6]) .and. !Empty(cUnCom )
				aadd(aItens,{;
							"",;
							"",;
							"",;
			...
			Elseif !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
				If cUnCom <> ""
					aadd(aItens,{;
