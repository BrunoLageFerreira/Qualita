...
local cICMSZFM      := ""
Local cMensCpl		:= ""
Local cCodCST		:= Alltrim(Upper(GetNewPar("MV_CODCST", "DF;PR=90;RJ;RS=10,90"))) // Bruno Lage + UF=CST,CST ADD ES ADICIONADO ES NO PARAMETRO
Local cCsosn2		:= ""
Local cBarra 	    := ""
Local cBarTrib 	    := ""
						...
						*/
                        ...
                        Else
                            aadd(aProd,	{Len(aProd)+1,;
                                ...
                                (cAliasSD2)->D2_QUANT,;
                                IIF(!((cAliasSD2)->D2_TIPO$"IP" .Or. ((cAliasSD2)->D2_TIPO $ "D" .And. cTpOrig == "P")) ,IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR-nDesVrIcms,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC+nDesVrIcms)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
                                cUmDipi ,;    // Bruno Lage retUn2UM( lNoImp2UM, lImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), cUmDipi, SB1->B1_UM ),;
                                IIf(cF2Tipo == "B",retQtd2UM( lNoImp2UM, lImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), nConvDip, (cAliasSD2)->D2_QUANT, SB1->B1_TIPCONV ),U_RETPESOFAT((cAliasSD2)->D2_PEDIDO,(cAliasSD2)->D2_ITEMPV)  ),; 			// BRUNO SIGAWISE retQtd2UM( lNoImp2UM, lImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), nConvDip, (cAliasSD2)->D2_QUANT, SB1->B1_TIPCONV ),;
                                (cAliasSD2)->D2_VALFRE,;
                                (cAliasSD2)->D2_SEGURO,;
                                (nDesconto+nDescIcm+nDescRed+nDescNfDup+nDescFis),;
                                0,;// O valor unitario sera obtido pela divisao do valor do produto pela quantidade comercial de acordo com o  Manual do Contribuinte 6.00 realizado na tag <vUnCom>(ConvType(aProd[10]/aProd[09],21,8)) 
...
If ExistBlock("PEGRNFESEFAZ")   
	...
	If AllTrim(aNota[1]) = "2"
		...
		aParam := {aNota,aProd,aIcms,aIcmsSt,aIpi,aICMUFDest,aPis,aCOFINS,aExp,aEspVol,aICMSZFM,aNfVinc,aIPIDevol,aCstIpi}
...
If((!Empty(aNfVinc)	.or. !empty(aNfVCdd)) .And. Empty(aExp[1])) .or.(!Empty(aNfVinc).And. !Empty(aExp[1]) /*.and. lEECFAT*/) // Bruno Lage 
					...
					If !(aNfVinc[Nx][7] $ cChvDupli)	
                        ...
                        if !Empty(aNfVinc[Nx][6]) .and. "CTE" == UPPER(Alltrim(aNfVinc[Nx][6]))
							...
							If ! '<refNFe>'+aNfVinc[Nx][7]+'</refNFe>' $ cString 	//Bruno Lage
								cString += '<refNFe>'+aNfVinc[Nx][7]+'</refNFe>'    // cString += refnfesig(cTpNota,aNfVinc[Nx][7],aNfVinc[Nx][6]) 
							EndIf 
                        EndIf 
...
ElseIf !Empty(aExp[1]) .and. !lEECFAT .and. (aNota[04] == "1" .or. (aNota[04] == "0" .and. aNota[5] $ "D|N"))  //.and. Empty(aNfVinc)
	For nX := 1 To Len(aExp)
		...
		Endif	
	Next nX
	If ! cChaveRef $ cString 	//Bruno Lage !Empty(cChaveRef)
		cString += '<NFRef>'
		cString += cChaveRef
		cString += '</NFRef>'
	EndIf
...
cString += '<qCom>'+ConvType(round(aProd[09],4),15,4 )+'</qCom>' // bruno original  cString += '<qCom>'+ConvType(aProd[09],15,4)+'</qCom>'

/*
bruno sigawise
-original = cString += '<vUnCom>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],23,10))+'</vUnCom>'
-não tem o if
*/
If aProd[8] <> aProd[11]
	cString += '<vUnCom>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],23,10))+'</vUnCom>'
Else
    cString += '<vUnCom>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,8))+'</vUnCom>'
EndIf
...
If aProd[8] <> aProd[11]  // aProd[8] = B1_UM  e  aProd[11] = B5_UMDIPI - pega diferente para segunda unidade de medida 
	cString += '<qTrib>' + ConvType(aProd[12], 15, 4) + '</qTrib>' //original Bruno
Else
